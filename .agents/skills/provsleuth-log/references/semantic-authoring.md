...
2. Inspect every configured ontology lock and its exact-byte status. Do not fetch `owl:imports`,
   ...
   verifies supplied bytes and schema only: its term index is project-supplied, and ProvSleuth does
   not verify that the index was correctly or completely extracted from RDF/OWL.
3. Run `provsleuth --config <config> ontology-candidates <query> --json`. Preserve the returned
...
6. Submit the minimal proposal with `provsleuth --config <config> map-term <proposal.json> --actor
   <agent-id> [--language <same>] [--limit <same>] --json`. Supply only the local term identity, candidate-search identity, target or
   unmapped decision, concise rationale, limitations, and reported provenance. ProvSleuth preserves
...
7. Stop after proposal. A different actor may inspect it with `provsleuth mappings --json` and run
   `provsleuth review-mapping <mapping-id> --state accepted|rejected|contested --actor <reviewer>`.
...
8. Do not treat acceptance as activation. A human writes a request containing exact accepted
   mapping leaf IDs plus a public note, runs `provsleuth compile-semantic-policy <request.json>
   --actor <policy-owner> --json`, reviews the stored release, then pins its returned ID in
   `provsleuth.config.json`. Never select "all latest."
9. Run `provsleuth semantic-status --json` and `provsleuth check --strict --json`. Report stale or
...
4. Express rules only in ProvSleuth's finite, function-free, typed rule language. Do not add remote
