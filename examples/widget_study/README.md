...
A complete, synthetic ProvSleuth project. The "science": does polishing a widget longer make it
...
This fixture was recorded before the project was renamed. Its existing content-addressed records
and graph extractors retain legacy `claimtrace/` paths and `claimtrace.*` identifiers. Use the
current `provsleuth` command with this legacy project. Do not rename the existing paths or records
in place; commands that create receipts, reviews, derivations, or snapshots append new records
under the legacy-configured store.

## Run it

```bash
cd examples/widget_study
provsleuth run --input data/raw_measurements.csv --input analysis/01_clean.py \
  --output data/clean.csv -- python analysis/01_clean.py
provsleuth run --input data/clean.csv --input analysis/02_fit.py \
  --output results/fit.json -- python analysis/02_fit.py
provsleuth run --input data/clean.csv --input results/fit.json --input analysis/03_figure.py \
  --output figures/fit.svg -- python analysis/03_figure.py

provsleuth check        # OK — all paths exist, nothing stale, nothing on a retired branch
provsleuth check --strict --json  # also audits whether these runs genuinely produced the outputs
provsleuth verify       # PASS — slope ≈ 2.0 and R2 > 0.9, read live from results/fit.json
provsleuth assessments  # accepted support plus an explicit causal-language narrowing
provsleuth snapshot     # lock figures/fit.svg's input hashes into a manifest
provsleuth evidence-plan pred:slope --json
provsleuth derive symbolic-positive-slope.proposal.json --actor widget-demo-agent --json
provsleuth derivations --json
provsleuth view --output research-map.html
```
...
outputs, and then run the wrapped commands above so ProvSleuth observes genuine missing-to-produced
...
binding. ProvSleuth obtains the required binding, vocabulary, rule pack, target, predicate,
polarity, extractors, and exact slope value from project-owned files:

```bash
provsleuth evidence-plan pred:slope --json
provsleuth derive symbolic-positive-slope.proposal.json --actor widget-demo-agent --json
provsleuth explain proof:sha256:<digest> --json
```

`widget-demo-agent` is a synthetic fixture actor. In a real project, pass the truthful identity of
the user or agent requesting plan materialization; ProvSleuth records the value but does not
authenticate it.
...
```bash
provsleuth assessments --json       # current accepted leaves and live findings
provsleuth assessments --all --json # proposals plus their immutable review decisions
```

For a new assessment, an external agent writes only `claim_id`, `result_ids`, and `agent_input`,
then submits it with `provsleuth assess proposal.json --actor <agent-id>`. A separate actor reviews
the returned content-addressed ID with:

```bash
provsleuth review assessment:sha256:<digest> --state accepted --actor <reviewer-id>
```

ProvSleuth itself computes node/file hashes, resolves the exact evidence anchors, and derives the
eligible relation. Edit `results/fit.json` after review and `provsleuth assessments` will mark both
...
provsleuth check        # STALE_DATA on fig:fit — its locked input (clean.csv? raw) changed since snapshot
```

(Re-run the three scripts + `provsleuth snapshot` to go green again.)

## See the propagation list

```bash
provsleuth impact --set dataset_version=v3
...
```bash
provsleuth journal                  # groups every node by verdict
provsleuth journal --status dead_end  # shows exp:loglog — a log-log fit that was tried and abandoned
```
