...
STAGE_TRACE_PATH_ENV = "CLAIMTRACE_STAGE_TRACE_PATH"
STAGE_TRACE_NONCE_ENV = "CLAIMTRACE_STAGE_TRACE_NONCE"
STAGE_TRACE_CONTRACT_ENV = "CLAIMTRACE_PIPELINE_CONTRACT_ID"
STAGE_TRACE_ROOT_ENV = "CLAIMTRACE_STAGE_SOURCE_ROOT"
...
def _stage_trace_environment() -> dict[str, str] | None:
    values = {
        "path": os.environ.get(STAGE_TRACE_PATH_ENV),
        "nonce": os.environ.get(STAGE_TRACE_NONCE_ENV),
        "contract_id": os.environ.get(STAGE_TRACE_CONTRACT_ENV),
        "source_root": os.environ.get(STAGE_TRACE_ROOT_ENV),
    }
    ...
    if present != set(values):
        raise PipelineError(
            "Claimtrace stage-checkpoint environment is incomplete; refusing a partial trace"
        )
    if not Path(values["path"]).is_absolute():
        raise PipelineError("Claimtrace stage-checkpoint path must be absolute")
    if not _STAGE_TRACE_NONCE_RE.fullmatch(values["nonce"]):
        raise PipelineError("Claimtrace stage-checkpoint nonce is invalid")
    if not CONTRACT_ID_RE.fullmatch(values["contract_id"]):
        raise PipelineError("Claimtrace stage-checkpoint contract id is invalid")
    if not Path(values["source_root"]).is_absolute():
        raise PipelineError("Claimtrace stage-checkpoint source root must be absolute")
    return values
...
def stage_checkpoint(stage_id: str) -> bool:
    ...
    Outside a Claimtrace-instrumented child this is a no-op and returns ``False``.
