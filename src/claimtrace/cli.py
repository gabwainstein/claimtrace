...
def cmd_lock_ontology(args):
    ...
    if args.json:
        ...
        print(f"claimtrace lock-ontology: {lock['id']}")
...
def cmd_ontology_candidates(args):
    ...
    if args.json:
        ...
        print(
            f"claimtrace ontology-candidates: {len(candidates['candidates'])} shown / "
...
def cmd_map_term(args):
    ...
    if set(entry) != {"terminology_id", "term_id", "agent_input"}:
        raise SemanticError(
            ...
            "agent_input; snapshots, review, and derived fields are computed by Claimtrace"
        )
    ...
    if args.json:
        ...
        print(f"claimtrace map-term: recorded {document['id']}")
...
def cmd_mappings(args):
    ...
    if args.json:
        ...
        print(f"claimtrace mappings: {len(items)} mapping record(s), "
...
def cmd_review_mapping(args):
    ...
    if args.json:
        ...
        print(f"claimtrace review-mapping: {document['id']}")
        print(
...
def cmd_compile_semantic_policy(args):
    ...
    if args.json:
        ...
        print(f"claimtrace compile-semantic-policy: recorded {document['id']}")
...
def cmd_replay(args):
    ...
    if args.json:
        ...
        print(f"claimtrace replay: {certificate['outcome']}  {certificate['id']}")
...
def cmd_assess_method(args):
    ...
    if args.json:
        ...
        print(f"claimtrace assess-method: recorded {document['id']}")
...
def cmd_method_assessments(args):
    ...
    if args.json:
        ...
        print(f"claimtrace method-assessments: {len(statuses)} current assessment(s); integrity {payload['integrity']}")
...
def cmd_review_method(args):
    ...
    if args.json:
        ...
        print(f"claimtrace review-method: {args.assessment_id} -> {args.state} by {args.actor}")
...
def cmd_graph_propose(args):
    ...
    if args.json:
        ...
        print(f"claimtrace graph-propose: {proposal['proposal_id']}")
...
def cmd_graph_apply(args):
    ...
    if args.json:
        ...
        print(f"claimtrace graph-apply: {result['status']}  {result['proposal_id']}")
...
def cmd_release_create(args):
    ...
    if args.json or output is None:
        ...
        print(f"claimtrace release-create: {document['release_id']}")
...
def cmd_release_verify(args):
    ...
    if args.json:
        ...
        print(f"claimtrace release-verify: {'valid' if result['valid'] else 'drifted'}")
...
def cmd_release_diff(args):
    ...
    if args.json:
        ...
        print(f"claimtrace release-diff: {'equal' if result['equal'] else 'different'}")
...
EXAMPLE_GRAPH = {
    ...
    "nodes": [
        {"id": "data:example", "type": "data", "status": "current", "backbone": "v1",
         "path": "data/claimtrace-example.csv", "value": "two example measurements"}
    ],
    "edges": [],
}
EXAMPLE_DATA = "measurement\n1\n2\n"
EXAMPLE_VERIFIERS = '''"""Checks for the explicit Claimtrace example scaffold."""
...
def row_count():
    ...
    import csv
    with open("data/claimtrace-example.csv", encoding="utf-8", newline="") as f:
...
def _init_locked(base: Path, args) -> int:
    ...
    trace = base / "claimtrace"
    gp = trace / "graph.json"
    vp = trace / "verifiers.py"
    dp = base / "data" / "claimtrace-example.csv"
    config_content = json.dumps(
        {"root": ".", "graph": "claimtrace/graph.json",
         "verifiers": "claimtrace/verifiers.py" if example else None,
    ...
    scaffold_files = [(gp, graph_content, "Claimtrace graph")]
    if example:
        scaffold_files.extend((
            (vp, EXAMPLE_VERIFIERS, "Claimtrace verifier"),
            (dp, EXAMPLE_DATA, "Claimtrace example data"),
        ))
    try:
        _assert_safe_publication_target(base, cfg_path, "Claimtrace config")
    except _UnsafePublicationPath as exc:
        print(f"claimtrace init: refusing unsafe path: {_terminal_text(exc)}", file=sys.stderr)
        return 1
    ...
    if cfg_path.exists() and not args.force:
        ...
        return 1
    try:
        _ensure_safe_publication_directory(base, trace, "claimtrace scaffold")
    ...
    try:
        config_state = _publish_text(
            base, cfg_path, config_content, force=args.force, label="Claimtrace config",
        )
    except (_PublicationConflict, _UnsafePublicationPath, OSError) as exc:
        print(f"claimtrace init: refusing to overwrite config: {_terminal_text(exc)}",
              file=sys.stderr)
        return 1
    ...
    for destination, content, label in scaffold_files:
        ...
        try:
            ...
            if args.force:
                print(f"claimtrace init: cannot publish scaffold: {_terminal_text(exc)}",
                      file=sys.stderr)
                return 1
        except (_UnsafePublicationPath, OSError) as exc:
            print(f"claimtrace init: refusing unsafe path: {_terminal_text(exc)}",
                  file=sys.stderr)
            return 1
    installed = [CONFIG_NAME, "claimtrace/graph.json"]
    if example:
        installed.extend(("claimtrace/verifiers.py", "data/claimtrace-example.csv"))
    print(f"initialised {'example' if example else 'planning'} claimtrace in {base}\n  "
          + "\n  ".join(installed))
    if example:
        print("Next: run `claimtrace verify`, then `claimtrace check --strict`.")
    else:
        print("Next: run `claimtrace check --strict`; add nodes only when their real "
...
def cmd_init(args):
    ...
    try:
        ...
        print(f"claimtrace init: cannot create project directory: {_terminal_text(exc)}",
              file=sys.stderr)
        return 1
    try:
        ...
        print(f"claimtrace init: cannot acquire project scaffold lock: "
...
def _packaged_skill_files():
    root = resources.files("claimtrace") / "templates" / "claimtrace-log"
    return {
...
def _install_skill_locked(base: Path, args) -> int:
    ...
    choices = {
        "agents": base / ".agents" / "skills" / "claimtrace-log",
        "claude": base / ".claude" / "skills" / "claimtrace-log",
    }
    ...
    try:
        ...
        print("claimtrace install-skill: refusing unsafe path:", file=sys.stderr)
    ...
    if verification_errors:
        print("claimtrace install-skill: final bundle verification failed:", file=sys.stderr)
    ...
    for destination, _content, state in published:
        ...
        print(f"claimtrace install-skill: {action}  {destination}")
    return 0
...
def cmd_install_skill(args):
    ...
    try:
        ...
        print(f"claimtrace install-skill: cannot create project directory: {_terminal_text(exc)}",
              file=sys.stderr)
        return 1
    try:
        ...
        print(f"claimtrace install-skill: cannot acquire project install lock: "
              f"{_terminal_text(exc)}", file=sys.stderr)
        return 1
...
def main(argv=None):
    ...
    p.add_argument("entry", help="claimtrace method-conformance proposal JSON")
    ...
    p = sub.add_parser("init", help="scaffold a planning-safe Claimtrace project")
    ...
    try:
        ...
        print(f"claimtrace: {_terminal_text(e)}", file=sys.stderr)
        return 2
