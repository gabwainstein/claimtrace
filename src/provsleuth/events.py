"""Content-addressed mechanical run receipts for ProvSleuth.
...
def _event_lock(path: Path, timeout: float = 30.0):
    # Frozen across the product rename so Claimtrace-era and ProvSleuth
    # processes cannot bypass one another's append lock.
...
def run_command(
    ...
    *,
    ...
    Cooperative provsleuth runs that declare the same output are serialized across
...
def _run_command_locked(
    ...
    *,
    ...
    if control_changed:
        contract_errors.append(
            "provsleuth config, graph, or event ledger changed during child execution; "
