...
def create_derivation_from_bindings(
    ...
    The caller selects only project-declared result/binding identities. ProvSleuth
...
def derivations_path(cfg) -> Path:
    ...
    prefix = getattr(cfg, "store_prefix", "provsleuth")
    configured = logic.get("derivations", f"{prefix}/derivations")
...
def append_derivation(cfg, document: dict) -> Path:
    ...
    with _store_lock(root):
        ...
        descriptor, temporary_name = tempfile.mkstemp(
            prefix=".provsleuth-derivation-", suffix=".tmp", dir=root.parent,
        )
        try:
