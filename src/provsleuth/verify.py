...
`provsleuth check` validates the graph STRUCTURE (paths exist, no cycles/dangling edges, nothing
...
registered with the `@check` decorator, and `provsleuth verify` runs them.

    # verifiers.py  (path set in provsleuth.config.json -> "verifiers")
...
SECURITY: `provsleuth verify` imports and EXECUTES the project's own `verifiers.py` (a plugin
...
def run_verifiers(cfg) -> int:
    ...
    if not cfg.verifiers:
        print(
            "provsleuth verify: NOT_CONFIGURED - no verifiers configured "
            "(set 'verifiers' in provsleuth.config.json or the legacy project config)."
        )
        return 2
    vp = Path(cfg.verifiers)
    if not vp.exists():
        print(f"provsleuth verify: verifiers file not found: {vp}")
        return 1
    old = os.getcwd()
    os.chdir(cfg.root)                       # so relative data paths in checks resolve
    try:
        spec = importlib.util.spec_from_file_location("_provsleuth_verifiers", vp)
        mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(mod)         # importing registers the checks
        print("provsleuth verify — live numeric checks\n")
        oks = []
        ...
        if not oks:
            print(
                "provsleuth verify: NO_CHECKS - configured verifier registered zero checks."
            )
            return 2
        if all(oks):
            print(f"provsleuth verify: OK — {len(oks)} check(s) pass.")
            return 0
        print(f"provsleuth verify: FAIL — {oks.count(False)}/{len(oks)} check(s) drifted from their artifacts.")
        return 1
    finally:
        os.chdir(old)
